...
\begin{tikzpicture}
	...
	\node (TA) [minimum width=1cm, minimum height=0.5cm, left=0cm of A0] {$CP_i$ of $\tau_0$};
	\node (TB) [minimum width=1cm, minimum height=0.5cm, left=0cm of B0] {$CP_i$ of $\tau_1$};
	\node (TC) [minimum width=1cm, minimum height=0.5cm, left=0cm of C0] {$CP_i$ of $\tau_2$};
  
	\draw[->] (A0.south) -- (B3.north);
	\draw[->] (A1.south) -- (B0.north);
	\draw[->] (A2.south) -- (B4.north);
	\draw[->] (A3.south) -- (B2.north);
	\draw[->] (A4.south) -- (B1.north);
	
	\draw[->] (B0.south) -- (C4.north);
	\draw[->] (B1.south) -- (C3.north);
	\draw[->] (B2.south) -- (C1.north);
	\draw[->] (B3.south) -- (C2.north);
	\draw[->] (B4.south) -- (C0.north);
\end{tikzpicture}