...
Due to being a harmonic task-set and its properties (see \cref{sec:scheduling}), the \textit{planning cycle}\cite{dar-tzenpengAssignmentSchedulingCommunicating1997} of $\Gamma$ is defined by the \textit{\ac{LCM}} of all tasks' periods $\{P_1, \ldots, P_n\}$, resulting in the period $P_{max}$ of the task with the highest period. 
...
\todo{make sure the times fit. CT<1/2period <- is this correct? is this correct for mult 2 or at all? But spare time is needed, true.}
...
% This could be as simple as using modulo calculations or by any other hash function.
...
% \section{Tasks with Release Times and Execution Times}\label{sec:tasks_release_execution}
...
% \section{Creating Conditions}\label{sec:creating_conditions}
...
\todo{depict schedule,  reference execution model}
